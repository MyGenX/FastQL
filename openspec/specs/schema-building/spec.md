# schema-building Specification

## Purpose
Compiling registered decorator definitions into a validated Schema, resolving forward/circular references.
## Requirements
### Requirement: Construct schema from decorated roots

The framework SHALL provide `Schema(query=..., mutation=..., subscription=..., types=[...], config=...)`
as the canonical schema API and SHALL compile fresh IR from decorated definitions.
...
Zero-argument `build_schema()` SHALL discover decorated root classes and assemble each root operation
type by merging distinct fields contributed across multiple container classes. When two contributions
produce the same final GraphQL field name on the same operation type, the framework SHALL raise a
descriptive error identifying the duplicated field.

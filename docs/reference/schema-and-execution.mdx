...
is invalid or is not a subscription. See [Subscriptions](/advanced/subscriptions).
...
`@defer` / `@stream`. See [Incremental delivery](/advanced/incremental-delivery).

## Schema extensions

`Schema(query=..., extensions=[...])` (and `build_schema(..., extensions=[...])`)
attach [schema extensions](/advanced/schema-extensions) that observe the operation
